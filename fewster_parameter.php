<?PHP
	...
	class fewster_parameter{
		...
		function menu_create(){
			add_submenu_page( "fewster-anti-bad", "Scan", "Parameter Scan", "manage_options", "fewster-scan-ip", array($this, "scan") );
		}		
			
		function check_parameter($file){
			if($file != str_replace("\\","/",__FILE__)){
				if(strpos(file_get_contents($file), stripslashes($_GET['parameter']))){
					return $file;
				}
			}
		}	
	...
	$fewster_parameter = new fewster_parameter();
