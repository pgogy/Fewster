<?PHP
	...
	class fewster_plugin_list{
		...
		function scripts(){
			...
			wp_enqueue_script( 'fewster_quick_integrity', plugins_url() . "/fewster/js/quick_integrity.js" );
		...
		function plugin_lists($actions, $plugin_file, $plugin_data, $context){
			...
			$actions['integrity'] = "<span class='fewster_integrity_options'>" . __("Integrity check") . "</strong> <a href='" . admin_url("admin.php?page=fewster-plugin-check&name=" . $plugin_data['Name'] . "&plugin=") . str_replace("\\","/",ABSPATH) . 'wp-content/plugins/' . $parts[0] . "/'>" . __("Full") . "</a> | <a href=\"javascript:fewster_quick_integrity_data('" . strtolower(str_replace(" ","_", $plugin_data['Name'])) . "','" . str_replace("\\","/",ABSPATH) . 'wp-content/plugins/' . $parts[0] . "/')\">" . __("Quick") . "</a>";
